...
async def get_full_cart_details(user_id: int, db: AsyncSession):
...
async def add_item_to_cart(user_id: int, item_data: CartItemCreate, db: AsyncSession):
    # Відкриваємо транзакцію на самому початку
    async with db.begin():
        ...
        # 4. ВАЖЛИВО: Flush та Refresh всередині транзакції
        await db.flush()
